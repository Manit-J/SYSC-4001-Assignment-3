...
int sem_wait(int semid, int semnum, int nowait) {
    ...
    *shared_index = 0; // Initialize shared index
    ...
    for (int i = 1; i <= NUM_TAS; i++) {
        ...
        if (pid == 0) {
            ...
            while (1) {
                // Try to acquire semaphore j
                printf("TA%d is trying to acquire semaphore %d.\n", ta_id, ta_id - 1);
                sem_wait(semid, ta_id - 1, 0); // Blocking wait for semaphore j
                printf("TA%d acquired semaphore %d.\n", ta_id, ta_id - 1);

                // Try to acquire semaphore j+1 without blocking
                printf("TA%d is trying to acquire semaphore %d.\n", ta_id, ta_id % NUM_TAS);
                int ret = sem_wait(semid, ta_id % NUM_TAS, 1); // Non-blocking attempt for semaphore j+1
                if (ret == -1) {
                    // Semaphore j+1 not available, release semaphore j
                    printf("TA%d could not acquire semaphore %d, releasing semaphore %d.\n", ta_id, ta_id % NUM_TAS, ta_id - 1);
                    sem_signal(semid, ta_id - 1);
                    // Sleep for a short random time before retrying
                    sleep(rand() % 2 + 1);
                    continue; // Retry the loop
                }
                printf("TA%d acquired semaphore %d.\n", ta_id, ta_id % NUM_TAS);

                // Acquired both semaphores
                printf("TA%d has accessed the database.\n", ta_id);

                // Lock shared index semaphore
                sem_wait(semid, NUM_TAS, 0);
                ...
                sem_signal(semid, ta_id - 1);
                sem_signal(semid, ta_id % NUM_TAS);
                printf("TA%d has released semaphores %d and %d.\n", ta_id, ta_id - 1, ta_id % NUM_TAS);
