...
int main() {
    ...
    int semid = semget(IPC_PRIVATE, 1, IPC_CREAT | 0666); // Single semaphore for critical section
    if (semid == -1) {
    ...
    for (int i = 1; i <= NUM_TAS; i++) {
        ...
        if (pid == 0) {
            ...
            while (1) {
                ...
                printf("TA%d is trying to acquire the semaphore.\n", ta_id);
                sem_wait(semid, 0); // Blocking wait for the semaphore
                printf("TA%d has acquired the semaphore.\n", ta_id);
                ...
                // Release semaphore
                sem_signal(semid, 0);
                printf("TA%d has released the semaphore.\n", ta_id);
